...
int compare(Operator a, Operator b) {
    if (a == b) return 0;
...
//Put card to a premise at the given index.
...
//Apply tabula rasa to premise, remove the given variable card and store the pointer to that card in dest.
//Returns false if the designated card is not a variable card.
...
//Return false if the expression cannot be build (premise is not valid).
